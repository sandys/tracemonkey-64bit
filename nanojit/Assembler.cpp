...
//		RegisterMask prefer = livePastCall(_ins) ? saved : scratch;
	...
	{	
		...
		//sss uint32_t managed = regs->managed;
		Register r = FirstReg;
		while(managed)
		{
			if (managed&1)
...
#define countlir_call()
	...
	{
		// trace must end with LIR_x, LIR_loop, LIR_ret, or LIR_tramp
		NanoAssert(reader->pos()->isop(LIR_x) ||
		           reader->pos()->isop(LIR_loop) ||
		           reader->pos()->isop(LIR_ret) ||
				   reader->pos()->isop(LIR_tramp));
